...
import constant from "../src/pages/constant/constant";
import {showError} from "./pages/utils/index";
let oldRequest: any;
    ...
    if (err.response !== undefined) {
      if (err.response.data !== null) {
        if (err.response.data.message  && err.response.data.status === 400) {
          ...
          showError(msg1);
        } else if (err.response.data.message && err.response.data.status === 500) {
          const msg1 = err.response.data.message;
          showError(msg1);
        } else if (err.response.data.status === 415) {
          const msg1 = err.response.statusText;
          showError(msg1);
        } else if (err.response.data.status === 404 || err.response.data.status === 403) {
          const msg1 = err.response.data.message;
          showError(msg1);
        }
